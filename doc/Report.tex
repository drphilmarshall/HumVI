...
\end{equation}
for $\vec x$. We have an initial guess, $\vec{x_0}$; from this we compute a residual vector,
\begin{equation}
	\vec{r_0} = \vec{b} - \mathbf{A}\vec{x_0}.
\end{equation}
Now we can find a correction to $\vec{x_0}$ by solving the matrix equation
\begin{equation}
	\mathbf{A}\vec{\Delta x} = \vec{r_0}.
\end{equation}
Then our final estimate for the solution is
\begin{equation}
	\vec{x} = \vec{x_0} + \vec{\Delta x}.
\end{equation}
...
For our purposes, the initial guess will just be the observed image; or in the language used above, $\vec{x_0}=\vec{b}$. However, we can't simply plough ahead with this, because the original image and the target image will have different numbers of elements ($\mathbf A$ is not square). This is easily rectified by augmenting the original image vector $\vec b$ with zeroes and using this padded version as $\vec{x_0}$; thankfully there is no ``dishonesty'' here (see section~\ref{sec:shave}), since $\vec{x_0}$ is only a guess in the first place.

We still have to be a little careful about how we achieve this padding: we shouldn't put zeros in the wrong places. If the original image was a square of side $n$ pixels, and the kernel image a square of side $m$ pixels, then the initial guess image should have side $n+m-1$ pixels. The vector $\vec{x_0}$ must therefore be padded with $(n+m-1)^2-n^2$ zeroes, distributed equally at the beginning and end of each stride. This could be achieved by doing the following:

\texttt{img\_vec\_padded = img\_vec.append(zeros(floor(m/2)), [[i*stride, -i*stride] }\\~\mbox{}\hspace{10cm}\texttt{for i in range(len(img\_vec))])}
