...
\begin{itemize}
	\item It takes a long time to solve for the image. Even something of 50x50 pixels takes ten minutes on my computer.
...
\begin{itemize}
	\item For Gaussian data (large) and a Gaussian kernel (small), find the scene (medium).
	\item For Gaussian data (large) and a Gaussian scene (medium), find the kernel (small).
\end{itemize}
Can solve these exactly, so compare the results with theory predictions.

\TODO{Cato: this is what I'm already doing, but the kernel does not come out to be a Gaussian (\ref{fig:decontest}). What's going on?}
