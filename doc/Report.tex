...
$\mathbf B$ is then a matrix with ${\rm size}(a)$ rows and ${\rm size}(k)$ columns. The vector problem in equation~\ref{eqn:vector} is overdetermined, so we must use some optimisation procedure (typically least-squares minimisation) to find $\vec k$.
...
Once we have constructed an appropriate kernel matrix, we solve the linear system to find the pixel vector $\vec b$ for the target image $\mathcal B$.

\TODO{Cato: use \texttt{scipy.sparse}\ldots}

\TODO{Cato: pseudocode?}

\TODO{Cato: I think this stuff is no longer relevant}
...
\texttt{\begin{enumerate}
	\item img\_array = readin(imagefile)	\#\# Or can be given as straight array
	\item img\_vec = img\_array.flatten()
	\item krn\_array = readin(kernelfile)	\#\# Or can be given as straight array
	%\item \MORE?
	\item krn\_mat = sparse\_diag\_matrix(krn\_array.flatten(), offsets=[0,1,2,\ldots])	\#\# general gist
	\item refimg\_vec = sparse\_solve(krn\_mat, img\_vec)
	\item refimg\_vec = stretch(refimg\_vec)	\#\# Ease of viewing
	\item refimg = reshape(refimg\_vec)	\#\# Make into an image shape
